<!-- Main modal -->
<div id="popup-edit-{{ $item->id_jaksa }}" tabindex="-1" aria-hidden="true"
    ...
    <div class="relative p-4 w-full max-w-2xl h-full md:h-auto">
        ...
        <div class="relative p-4 bg-white rounded-lg shadow sm:p-5">
            ...
            <!-- Modal body -->
            <form method="post" action="{{ route('admin.jaksa.update', $item->id_jaksa) }}" autocomplete="off">
                @method('put')
                @csrf
                <div class="flex mb-4">
                    <label for="nip"
                        class="flex-shrink-0 z-10 inline-flex items-center py-2.5 px-4 text-sm font-medium text-center text-gray-900 bg-gray-100 border border-e-0 border-gray-300 rounded-s-lg hover:bg-gray-200 focus:ring-4 focus:outline-none focus:ring-gray-300 w-40">Nip</label>
                    <div class="relative w-full">
                        <input type="text"
                            ...
                            id="nip" name="nip" value="{{ $item->nip }}" required />
                    </div>
                </div>
                <div class="flex mb-4">
                    <label for="nama"
                ...
                <div class="flex mb-4">
                    ...
                    <select id="pangkat" name="pangkat"
                        ...
                        @foreach ($pangkat as $pkt)
                            <option value="{{ $pkt }}" @if ($item->jabatan == $pkt) selected @endif>
                                {{ $pkt }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="flex mb-4">
                    ...
                    <div class="relative w-full">
                        <input type="text"
                            class="block p-2.5 w-full z-20 text-sm text-gray-900 bg-gray-50 rounded-e-lg rounded-s-gray-100 rounded-s-2 border border-gray-300 focus:ring-blue-500 focus:border-blue-500"
                            id="alamat" name="alamat" value="{{ $item->alamat }}" required />
                    </div>
                </div>
                <div class="flex mb-4">
                    <label for="email"
                        class="flex-shrink-0 z-10 inline-flex items-center py-2.5 px-4 text-sm font-medium text-center text-gray-900 bg-gray-100 border border-e-0 border-gray-300 rounded-s-lg hover:bg-gray-200 focus:ring-4 focus:outline-none focus:ring-gray-300 w-40">Email</label>
                    <div class="relative w-full">
                        <input type="email"
                            class="block p-2.5 w-full z-20 text-sm text-gray-900 bg-gray-50 rounded-e-lg rounded-s-gray-100 rounded-s-2 border border-gray-300 focus:ring-blue-500 focus:border-blue-500"
                ...
                <div class="flex mb-4">
                    ...
                    <div class="relative w-full">
                        <input type="text"
                            class="block p-2.5 w-full z-20 text-sm text-gray-900 bg-gray-50 rounded-e-lg rounded-s-gray-100 rounded-s-2 border border-gray-300 focus:ring-blue-500 focus:border-blue-500"
                            id="notelp" name="notelp" value="{{ $item->notelp }}" required />
                    </div>
                </div>
                <div class="flex space-x-1 items-center pb-4 pt-4 mt-4 mb-4 rounded-t border-b border-t sm:mb-5">
                    ...
                    <button data-modal-hide="popup-edit-{{ $item->id_jaksa }}" type="button"
