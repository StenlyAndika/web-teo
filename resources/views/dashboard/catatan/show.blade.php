<!-- Main modal -->
<div id="popup-edit-{{ $item->id_catatan }}" tabindex="-1" aria-hidden="true"
    ...
    <div class="relative p-4 bg-white rounded-lg shadow sm:p-5">
        ...
        <div class="flex justify-between items-center pb-4 mb-4 rounded-t border-b sm:mb-5">
            ...
            <button type="button"
                ...
                data-modal-toggle="popup-edit-{{ $item->id_catatan }}">
        ...
        <!-- Modal body -->
        <form method="post" action="{{ route('admin.catatan.update', $item->id_catatan) }}" autocomplete="off">
            @method('put')
            @csrf
            <div class="flex mb-4">
            ...
            <div class="flex mb-4">
                ...
                <select id="id_jaksa" name="id_jaksa"
                    ...
                    @foreach ($jaksa as $kont)
                        <option value="{{ $kont->id_jaksa }}" @if ($item->id_jaksa == $kont->id_jaksa) selected @endif >{{ $kont->nama_jaksa }}</option>
                    @endforeach
                </select>
            </div>
            <div class="flex mb-4">
                ...
                <div class="relative w-full">
                    ...
                    <input datepicker datepicker-autohide datepicker-format="yyyy-mm-dd" name="tgl_catatan" required type="text"
                        ...
                        placeholder="Pilih Tanggal" value="{{ Carbon\Carbon::parse($item->tgl_catatan)->format('Y-m-d') }}">
                </div>
            </div>
            <div class="w-full mb-4 border border-gray-200 rounded-lg bg-gray-100 dark:bg-gray-700 dark:border-gray-600">
                ...
                <div class="px-4 py-2 bg-white rounded-b-lg dark:bg-gray-800">
                    <textarea id="editor" name="isi_catatan" rows="8" class="block w-full px-0 text-sm text-gray-800 bg-white border-0 dark:bg-gray-800 focus:ring-0 dark:text-white dark:placeholder-gray-400" placeholder="Write an article..." >{{ $item->isi_catatan }}</textarea>
                </div>
            </div>
            <button type="submit"
